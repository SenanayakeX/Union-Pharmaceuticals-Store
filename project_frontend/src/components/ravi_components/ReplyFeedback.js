...
export function withRouter(Children){
    ...
    onSubmit = (e) => {
        ...
        const {feedbackid,name,email,date,contactno,feedbacktype,feedbackorinquiry,reply} = this.state;
        ...
        axios.get(`http://localhost:8070/feedback/${id}`).then((res) =>{
            if(res.data.success){
                this.setState({
                    ...
                    reply:res.data.feedbacks.reply,
...
<center><h1 style={{color:"blue"}}>Feedback/Inquiry</h1></center>
    <div className="form-group"><center>
        ...
        <input type="text" className="form-control" id="feedbackid" disabled placeholder="Enter Feedback Id"
         onChange={this.handleInputChange} value={this.state.feedbackid} Required = 'required' />
    ...
    <div className="form-group"><center>
         ...
         <input type="text" className="form-control" id="reply"  placeholder="Enter  Reply"
           onChange={(e)=>{this.setState({reply:e.target.value})}} defaultvalue={this.state.reply}  Required = 'required'/>
